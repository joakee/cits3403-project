...
with app.app_context():
    ...
    wl1 = Wishlist.query.filter_by(user_id=u1.id).first()
    if not wl1:
        wl1 = Wishlist(name='Favourites', user_id=u1.id)
        db.session.add(wl1)
        db.session.flush()

    wl2 = Wishlist.query.filter_by(user_id=u2.id).first()
    if not wl2:
        wl2 = Wishlist(name='Favourites', user_id=u2.id)
        db.session.add(wl2)
        db.session.flush()

    wl1.listings.append(l3)  # Alice wants Bob's laptop
    wl2.listings.append(l1)  # Bob wants Alice's bike
    db.session.commit()
